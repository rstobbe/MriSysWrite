...
function [RECONWRT,err] = WrtRecon_OldVarianTpiS_v1a_Func(RECONWRT,INPUT)
...
KSMP = IMP.KSMP;
TSMP = IMP.TSMP;
WRT = rmfield(WRT,{'GQNT','PSMP','GWFM','TSMP','KSMP','Kend','G','DES'});
WRT.KSMP.gcoil = KSMP.gcoil;
WRT.KSMP.graddel = KSMP.graddel;
WRT.TSMP.filtBW = TSMP.filtBW;

WRT.name = WRTSYS.name;
WRT.Kmat = single(IMP.Kmat(TORD.projsampscnr,:,:,:));
