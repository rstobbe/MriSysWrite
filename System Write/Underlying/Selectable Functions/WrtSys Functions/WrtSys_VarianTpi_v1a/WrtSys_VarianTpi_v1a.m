...
function [SCRPTipt,SYSWRT,err] = WrtSys_VarianTpi_v1a(SCRPTipt,SYSWRTipt)
...
SYSWRT.wrtrefocusfunc = SYSWRTipt.('WrtRefocusfunc').Func;  
...
REFOCWRTipt = SYSWRTipt.('WrtRefocusfunc');
if isfield(SYSWRTipt,([CallingLabel,'_Data']))
    if isfield(SYSWRTipt.([CallingLabel,'_Data']),'WrtRefocusfunc_Data')
        REFOCWRTipt.('WrtRefocusfunc_Data') = SYSWRTipt.([CallingLabel,'_Data']).('WrtRefocusfunc_Data');
    end
end
...
func = str2func(SYSWRT.wrtrefocusfunc);           
[SCRPTipt,REFOCWRT,err] = func(SCRPTipt,REFOCWRTipt);
if err.flag
    return
end
