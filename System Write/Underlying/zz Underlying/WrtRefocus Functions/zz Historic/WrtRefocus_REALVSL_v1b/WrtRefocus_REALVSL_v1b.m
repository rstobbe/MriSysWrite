...
function [SCRPTipt,WRT,err] = WrtRefocus_REALVSL_v1b(SCRPTipt,WRT)

Status('busy','Write Parameter File');

err.flag = 0;
err.msg = '';

RefocusDefLoc = WRT.wrtrefocusfunc.Struct.('RefocusDefLoc');

%-------------------------------------------------
% Name Parameter File
%-------------------------------------------------
[path0] = uigetdir(RefocusDefLoc,'Create Folder');
if path0 == 0
    err.flag = 1;
    err.msg = 'Refocus Files Not Written';
    return
end
WRT.RefocusLoc = path0;
